...
class Daemon {
    ...
    static async start(room) {
        ...
        log(`Starting daemon with args: ${args.join(' ')}`);
        ...
        child.on('exit', (code) => log(`Daemon exited with code: ${code}`));
        child.unref();
        ...
        try {
            ...
            log(`Failed to write PID/status: ${err.message}`);
        }
    ...
    static async stop() {
        ...
        const status = {
            pid: null, room: null, rooms: [], startedAt: null, status: 'stopped'
        };
        fs.writeFileSync(STATUS_FILE, JSON.stringify(status, null, 2));
        return status;
    }
    ...
    static getStatus() {
        try {
            ...
            return {
                pid: null, room: null, rooms: [], startedAt: null,
    ...
    static addRoom(room) {
        ...
        fs.writeFileSync(COMMAND_FILE, JSON.stringify({
            action: 'add', room, timestamp: Date.now()
        }));
        return room;
    }
    ...
    static sendMessage(message) {
        ...
        fs.writeFileSync(COMMAND_FILE, JSON.stringify({
            action: 'send', message, timestamp: Date.now()
        }));
    }
